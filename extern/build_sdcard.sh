...
sudo dd if=./u-boot-socfpga/u-boot-with-spl.sfp of=${devname} bs=64k seek=0

# Copy the root filesystem
sudo tar -C sdcard/ext3 -xzf ./rootfs.tar.gz
# Copy the kernel and the linux kernal device tree blob
sudo cp ./linux/arch/arm/boot/zImage ./sdcard/fat32
sudo cp ./linux/arch/arm/boot/dts/socfpga_cyclone5_socdk.dtb ./sdcard/fat32
# Copy the fpga programming file
sudo cp ../linux/yellowPirat.rbf sdcard/fat32
# Copy the u-boot boot script which will program the FPGA during u-boot bootloading
sudo cp ../linux/u-boot.scr sdcard/fat32
pushd sdcard/fat32
